...
# $File:     rcs-keywords-hook-post-commit.py $
# $Rev:      2017-12-18 18:03:31 -0600 $
# $Rev:      2017-12-18 18:03:31 -0600 $
# $Source:   rcs-keywords-hook-post-commit.py $
...
##            param_num = param_num + 1
    ...
    if DEBUG_FLAG:
        sys.stderr.write('  Getting list of committed files\n')
...
def get_committed_files():
    ...
    try:
        ...
        if DEBUG_FLAG:
            ...
            dump_file_stream(stream_handle=cmd_stdout,
                             stream_description=
                             'STDOUT from get_committed_files')
            dump_file_stream(stream_handle=cmd_stderr,
                             stream_description=
                             'STDERR from get_committed_files')
    ...
    # it to the caller
        ...
        if err.returncode == 128:  # new repository
            ...
            if DEBUG_FLAG:
                ...
                dump_file_stream(stream_handle=cmd_stdout,
                                 stream_description=
                                 'STDOUT from get_committed_files error')
                dump_file_stream(stream_handle=cmd_stderr,
                                 stream_description=
                                 'STDERR from get_committed_files error')
            raise
